...
class NodePartitioner {
 ...
 public:
  ...
  NodePartitioner(size_t n, size_t m, uintE* o, uintT* indo, words& dn,
                  bool rmetis);
  ~NodePartitioner();
...
NodePartitioner::NodePartitioner(size_t n, size_t m, uintE* o, words& dn,
                                 bool rmetis = false) {
  _n = n;
...
NodePartitioner::NodePartitioner(size_t n, size_t m, uintE* o, uintT* indo,
...
NodePartitioner::NodePartitioner(size_t n, size_t m, uintE* o,
  ...
  std::sort(
      o_vec.begin(), o_vec.end(),
      [](const std::pair<long, uintE>& a, const std::pair<long, uintE>& b) {
        return a.second > b.second;
      });
  size_t i = 0;
  size_t occupied = 0;
  // std::cerr << "occupied <cap? " << occupied << "+" << o_vec[i].second <<
  // std::endl;
  while ((i < n)) {
  ...
  //  dram_nodes[o_vec[i].first] = true;
  //  occupied += o_vec[i].second;
  //  //std::cout << " in nvram => " << o_vec[i].first  << "," <<
  //  o_vec[i].second << std::endl; i++;
  //}
  ...
  std::cout << "Created node partitioner with capacity " << capacity
            << std::endl;
  // for (size_t i=0; i<5; i++) { std::cerr << "first 5 dram_node | " <<
  // dram_nodes[i] << std::endl; } for (size_t i=0; i<5; i++) { std::cerr <<
  // "first 5 partition_offsets | " << partition_offsets[i] << std::endl; } for
  // (size_t i=0; i<n; i++) { std::cerr << "dram_node | " << dram_nodes[i] <<
  // std::endl; } for (size_t i=0; i<n; i++) { std::cerr << "partition_offsets |
  // " << partition_offsets[i] << std::endl; }
}
...
NodePartitioner::NodePartitioner(size_t n, size_t m, uintE* o, uintT* indo,
  ...
  std::cout << "Using asymmetric graph...capacity is " << capacity << " out of "
            << 2 * m << std::endl;
  ...
  // dram_nodes[i] << std::endl; } for (size_t i=0; i<5; i++) { std::cerr <<
  // "first 5 partition_offsets | " << partition_offsets[i] << std::endl; } for
  // (size_t i=0; i<2*n; i++) { std::cerr << "dram_node | " << dram_nodes[i] <<
  // std::endl; } for (size_t i=0; i<2*n; i++) { std::cerr << "partition_offsets
  // | " << partition_offsets[i] << std::endl; }
}
