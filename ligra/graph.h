...
struct Uncompressed_Memhypergraph : public Deletable {
 public:
  ...
  Uncompressed_Memhypergraph(vertex* VV, vertex* HH, long nnv, long mmv,
                             long nnh, long mmh, void* _edgesV, void* _edgesH,
                             void* _inEdgesV = NULL, void* _inEdgesH = NULL)
      : V(VV),
        H(HH),
        nv(nnv),
        mv(mmv),
...
struct graph {
#ifdef DEBUG
  // these operate on a global level
  long dram_accesses = 0;
  long nvram_accesses = 0;
  // these operate on a vertexSubset level
  long edgeMapDenseForward = 0;
  long edgeMapDense = 0;
  long edgeMapSparse = 0;
  long edgeMapSparseNoFilter = 0;
  // these operate on the node level
  long edgeMapDenseNL = 0;
  long edgeMapDenseNLBreakEarly = 0;
#endif
  ...
  graph(vertex* _V, long _n, long _m, Deletable* _D, uintE* _flags)
      : V(_V), n(_n), m(_m), D(_D), flags(_flags), transposed(0) {}
  ...
  void transpose() {
    if ((sizeof(vertex) == sizeof(asymmetricVertex)) ||
      ...
      parallel_for(long i = 0; i < n; i++) { V[i].flipEdges(); }
...
struct hypergraph {
  ...
  hypergraph(vertex* _V, vertex* _H, long _nv, long _mv, long _nh, long _mh,
             Deletable* _D)
      : V(_V),
        H(_H),
        nv(_nv),
  ...
  void transpose() {
    if ((sizeof(vertex) == sizeof(asymmetricVertex)) ||
        (sizeof(vertex) == sizeof(compressedAsymmetricVertex))) {
      parallel_for(long i = 0; i < nv; i++) { V[i].flipEdges(); }
      parallel_for(long i = 0; i < nh; i++) { H[i].flipEdges(); }
      transposed = !transposed;
    }
  }
};
